  ...
  const [formData, setFormData] = useState({
    ...
    const mutation = useMutation({
        mutationFn: async({data,id})=>{
          ...
          const response = await axios.put(`${base_url}/api/aircraft/updatemanufacture/${id}`,data)
        ...
        onSuccess:()=>{
          ...
          setTimeout(()=>{
            navigate('/manufactureTable')
          },1000)
        }
      })

                // data retrive ....
                  const {data:getmanufacturedata} = useQuery({
                    queryKey :['getmanufacturedatakey'],
                    queryFn : async()=>{
                        const response = await axios.get(`${base_url}/api/aircraft/getmanufacture/${userid}`)
    ...
    <div className='aircraft-status border p-4 rounded shadow'>
      <form onSubmit={handleSubmit}>
        ...
        <div className='d-flex justify-content-end mt-2'>
          ...
          <input className="btn-border-close rounded px-4  p-1 mx-2" type="reset" value="Close" onClick={()=>navigate('/manufactureTable')} />
        </div>
      </form>
