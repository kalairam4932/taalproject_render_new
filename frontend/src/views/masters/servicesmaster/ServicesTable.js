...
const ServicesTable = () => {
  ...
  // const [toggledClearRows, setToggleClearRows] = React.useState(false);
    ...
    const fetchUsers = async () => {
        const response = await axios.get(`${base_url}/api/master/GETSERVICES`);
    ...
    const dldRHTABLE = async ({deleteid}) => {
        ...
        const response = await axios.delete(`${base_url}/api/master/GETSERVICES/${deleteid}`);
        return response.data;
    };  
  ...
  return (
    <div >
            ...
            <div className="d-flex justify-content-between align-items-center mb-1 mt-3  p-2 rounded shadow bg-card-color ">
                <h5 className="text-white fw-bold">Services List</h5>
                <div>
                    <button className="btn btn-primary me-2" onClick={()=>navigated('/SERVICEFORM')}>Add New</button>
                        ...
                        {users ?.length > 0 ? (
                            users.map((vendor, index) => (
                                <tr key={index} className={index % 2 === 0 ? "bg-primaryone text-white border p-2 rounded" : "bg-light"}>
                                    <td>{vendor.servicestype}</td>

                               

                                    <td>
                                        <div className='d-flex justify-content-center'>
                                            <button className="btn btn-light mx-2" onClick={()=>navigated(`/LHUPDATE/${vendor._id}`)}  >
